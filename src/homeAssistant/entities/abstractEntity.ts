...
export abstract class AbstractEntity {
    ...
    constructor(
        ...
        this.name =
            parentZones.length > 0 ? `${parentZones.map(({ zone }) => zone.name).join('/')}/${entityName}` : entityName;
    ...
    abstract get homeAssistantEntityConfig(): Record<string, string | number | boolean>;
    ...
    public announce(): Promise<void> {
        ...
        const topic = `${config.LITECOM2MQTT_HOMEASSISTANT_DISCOVERY_MQTT_TOPIC_PREFIX}/${this.homeAssistantEntityType}/${uniqueId}/config`;
        ...
        return this.mqttClient.publish(topic, JSON.stringify(payload), { retain: false });
    }
